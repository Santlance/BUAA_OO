...
public class MyUmlModelInteraction implements UmlClassModelInteraction {
    private MyUmlModel umlModel;

    public MyUmlModelInteraction() {
        this.umlModel = new MyUmlModel();
    }

    protected void initialModelInteraction(UmlElement[] elements,
                                           ArrayList<UmlElement>
                                                   elementContainer) {
        //为了避免顺序输入顺序造成的影响，先把所有的类对象构建好
        this.addClassInterface(elements, elementContainer);
        this.addInteraction(elementContainer);
    ...
    protected MyUmlModel getUmlModel() {
        return this.umlModel;
    }

    private void addInteraction(ArrayList<UmlElement> elementContainer) {
        Iterator<UmlElement> iterator = elementContainer.iterator();
        while (iterator.hasNext()) {
            UmlElement elem = iterator.next();
            switch (elem.getElementType()) {
                case UML_INTERACTION:
                    this.getUmlModel().addInteraction(elem);
                    iterator.remove();
                    break;
                default:
                    break;
